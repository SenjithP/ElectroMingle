...
export const adminGetElectricianDetails = async (req, res) => {
  try {
    ...
    return res.status(200).json({ electricianDetails:reversedElectricianDetails });
  } catch (error) {
    console.error(error); 
    return res
      .status(500)
      .json({ message: "Internal server error: Unable to fetch electrician details" });
  }
};
...
export const adminElectricianBlockUnblock = async (req, res) => {
  try {
    ...
    electricianDetail.electricianIsBlocked = !electricianDetail.electricianIsBlocked;
    await electricianDetail.save();

    const blockStatus = electricianDetail.electricianIsBlocked ? "blocked" : "unblocked";
    res.cookie("userjwt", "", {
      httpOnly: true,
      expires: new Date(0),
    });
    ...
    console.error(error);
    return res.status(500).json({
      message: "Internal server error: Unable to fetch or update electrician details",
    });
  }
};
